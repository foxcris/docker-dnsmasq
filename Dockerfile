...
#install dnsmasq
RUN apt-get update && apt-get -y upgrade && DEBIAN_FRONTEND=noninteractive apt-get install -y dnsmasq && apt-get clean

RUN mv /etc/dnsmasq.conf /etc/dnsmasq.conf_default
